...
public class LoginController implements CommunityConstant {
    ...
    //添加验证图片不能简单写在上述请求中，上述请求会返回一个html,而此html会包含一个图片路径，浏览器会根据路径再次访问服务器获得该图片
    //因此需要再次写一个请求
    @RequestMapping(path="/kaptcha",method = RequestMethod.GET)
    //返回的是图片，比较特殊，需要自己手动response返回
...
//        session.setAttribute("kaptcha", text);
     ...
     * @param model 数据
     * @param session 会话，记录了验证码用以校对
     * @param response 使用cookie将ticket传给浏览器
     *
     */
